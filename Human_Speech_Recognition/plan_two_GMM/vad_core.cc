...
WebRtcVadCore::WebRtcVadCore()
{
    m_pVadSP = new WebRtcVadSP();
    m_pVadFilterBank = new WebRtcVadFilterBank();
    m_pVadGMM = new WebRtcVadGMM();
}
...
WebRtcVadCore::~WebRtcVadCore()
{
    ...
    if(m_pVadFilterBank) {
        delete m_pVadFilterBank;
        m_pVadFilterBank = NULL;
    }
    if(m_pVadGMM) {
        delete m_pVadGMM;
        m_pVadGMM = NULL;
...
// Initialize VAD
int WebRtcVadCore::WebRtcVad_InitCore(VadInstT *inst, short mode)
{
    int i;
...
int WebRtcVadCore::WebRtcVad_set_mode_core(VadInstT *inst, short mode)
{
    if (mode == 0)
    {
        // Quality mode
...
WebRtc_Word16 WebRtcVadCore::WebRtcVad_CalcVad32khz(VadInstT *inst, WebRtc_Word16 *speech_frame, int frame_length)
{
    ...
    m_pVadSP->WebRtcVad_Downsampling(speech_frame, speechWB, &(inst->downsampling_filter_states[2]),
                           frame_length);
    len = WEBRTC_SPL_RSHIFT_W16(frame_length, 1);

    m_pVadSP->WebRtcVad_Downsampling(speechWB, speechNB, inst->downsampling_filter_states, len);
...
WebRtc_Word16 WebRtcVadCore::WebRtcVad_CalcVad16khz(VadInstT *inst, WebRtc_Word16 *speech_frame, int frame_length)
{
    WebRtc_Word16 len, vad;
    WebRtc_Word16 speechNB[240]; // Downsampled speech frame: 480 samples (30ms in WB)

    // Wideband: Downsample signal before doing VAD
    m_pVadSP->WebRtcVad_Downsampling(speech_frame, speechNB, inst->downsampling_filter_states,
                           frame_length);
...
WebRtc_Word16 WebRtcVadCore::WebRtcVad_CalcVad8khz(VadInstT *inst, WebRtc_Word16 *speech_frame, int frame_length)
{
    WebRtc_Word16 feature_vector[NUM_CHANNELS], total_power;

    // Get power in the bands
    total_power = m_pVadFilterBank->WebRtcVad_get_features(inst, speech_frame, frame_length, feature_vector);
...
// hypothesis-test.
WebRtc_Word16 WebRtcVadCore::WebRtcVad_GmmProbability(VadInstT *inst, WebRtc_Word16 *feature_vector, WebRtc_Word16 total_power, int frame_length)
{
    int n, k;
    ...
    { // If signal present at all
        ...
        { // For all channels
            ...
            tmp32_1 = m_pVadGMM->WebRtcVad_GaussianProbability(xval, *nmean1ptr++, *nstd1ptr++,
                                                    &deltaN[pos]);
            probn[0] = (WebRtc_Word32)(kNoiseDataWeights[n] * tmp32_1);
            tmp32_1 = m_pVadGMM->WebRtcVad_GaussianProbability(xval, *nmean2ptr++, *nstd2ptr++,
                                                    &deltaN[pos + 1]);
            ...
            tmp32_1 = m_pVadGMM->WebRtcVad_GaussianProbability(xval, *smean1ptr++, *sstd1ptr++,
                                                    &deltaS[pos]);
            probs[0] = (WebRtc_Word32)(kSpeechDataWeights[n] * tmp32_1);
            tmp32_1 = m_pVadGMM->WebRtcVad_GaussianProbability(xval, *smean2ptr++, *sstd2ptr++,
                                                    &deltaS[pos + 1]);
            ...
            if (h0 > 0)
            {
                ...
                ngprvec[pos] = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(tmp32_2, h0);
            ...
            if (h1 > 0)
            {
                ...
                sgprvec[pos] = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(tmp32_2, h1);
        ...
        for (n = 0; n < NUM_CHANNELS; n++)
        {
            ...
            backval = m_pVadSP->WebRtcVad_FindMinimum(inst, feature_vector[n], n); // Q4
                ...
                *nmean2ptr = nmk3;
                ...
                if (vadflag)
                {
                    ...
                    *smean2ptr = smk2;
                    ...
                    // 0.1 * Q20 / Q7 = Q13
                    if (tmp32_2 > 0)
                        tmp16 = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(tmp32_2, ssk * 10);
                    else
                    {
                        tmp16 = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(-tmp32_2, ssk * 10);
                        tmp16 = -tmp16;
                ...
                } else
                {
                    ...
                    // Q20 / Q7 = Q13
                    tmp16 = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(tmp32_1, nsk);
                    if (tmp32_1 > 0)
                        tmp16 = (WebRtc_Word16)SignalProcessing::GetInstance()->WebRtcSpl_DivW32W16(tmp32_1, nsk);
                    else
                    {
